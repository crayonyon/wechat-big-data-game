#!/usr/bin/env python 
# -*- coding: utf-8 -*-
# @Time    : 2021/6/17 21:16
# @Author  : Lin Xiaolin
# @Site    : 
# @File    : baseline_lgb.py
# @Software: PyCharm
...
def main():
    ...
    ## 合并处理
    df = pd.concat([train, test], axis=0, ignore_index=True)  # 会有NaN
    print(df.head(3))
    ...
    ## 统计历史5天的曝光、转化、视频观看等情况（此处的转化率统计其实就是target encoding）    label 滑窗特征统计
    n_day = 5
    for stat_cols in tqdm([
        ...
        for target_day in range(2, max_day + 1):
            ...
            # feats= ['_userid_2day_count', '_userid_2day_finish_rate']
            ...
            # feats = ['_userid_2day_count', '_userid_2day_finish_rate', '_userid_2day_play_times_max', '_userid_2day_play_times_mean']
    ...
    file_name = 'submit one of 5 lgb %.6f_%.6f_%.6f_%.6f_%.6f.csv' % (
        weighted_uauc, uauc_list[0], uauc_list[1], uauc_list[2], uauc_list[3])

    file_name = cur_day + " " + file_name
