...
def parse_args():
    parser = argparse.ArgumentParser(description='config the model hyper parameter.')
...
if __name__ == "__main__":
    ...
    for action in ACTION_LIST:
        # USE_FEAT: 'userid', 'feedid', action, 'authorid', 'videoplayseconds', 'bgm_song_id', 'bgm_singer_id'
        #USE_FEAT = ['userid', 'feedid', action] + FEA_FEED_LIST[1:]
        # USE_FEAT: 'userid', 'feedid', action, device, 'authorid', 'videoplayseconds', 'bgm_song_id', 'bgm_singer_id'

        # USE_FEAT: 'userid', 'feedid', action, device, 'authorid', 'videoplayseconds', 'bgm_song_id', 'bgm_singer_id'
        # 再加入的 统计信息作为特征
        STATISTIC_FEA = ['read_commentsum','likesum','click_avatarsum','forwardsum',
                         'read_commentsum_user','likesum_user','click_avatarsum_user','forwardsum_user']
        USE_FEAT = ['userid', 'feedid', action, 'device'] + FEA_FEED_LIST[1:] + STATISTIC_FEA
        train = pd.read_csv(ROOT_PATH + f'/train_data_for_{action}.csv')[USE_FEAT]

        # train col: 'userid', 'feedid', action, 'authorid', 'videoplayseconds', 'bgm_song_id', 'bgm_singer_id'
        train = train.sample(frac=1, random_state=42).reset_index(drop=True)
        print("train.shape after sample:", train.shape)

        print("posi prop:")       # 正样本比例
        print(sum((train[action]==1)*1)/train.shape[0])

        # test col: 'userid', 'feedid', device, 'authorid', 'videoplayseconds', 'bgm_song_id', 'bgm_singer_id'
        ...
        dense_features = ['videoplayseconds'] + STATISTIC_FEA
        ...
        # 代码实现基于[DeepCTR](https://github.com/shenweichen/DeepCTR-Torch)，只做了简单数据预处理，
        # 采用的是基本特征（离散特征：{'userid', 'feedid', 'authorid', 'bgm_song_id', 'bgm_singer_id'}，
        # 连续特征：{'videoplayseconds'}），单个任务逐个预测，大家可以尝试改进原有的模块，并尝试新的模型和新的建模方法。
        ...
        model = MyDeepFM(linear_feature_columns=linear_feature_columns, dnn_feature_columns=dnn_feature_columns,
                       task='binary',
                       l2_reg_embedding=1e-1, seed=args.seed, device=device)
    ...
    # 保存提交文件
    try:
        submit.to_csv("./submit1_base_deepfm " + cur_time + " seed " + str(args.seed) +".csv", index=False)
    except IOError:
        submit.to_csv("./submit1_base_deepfm" + " seed " + str(args.seed) + ".csv", index=False)